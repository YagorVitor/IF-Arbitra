...
import { StudentSelect, Student } from '../../components/ui/StudentSelect';
import { Star, Diamond, Info, Plus } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { useAuth } from '../../contexts/AuthContext';

export function MeuSexteto() {
  const navigate = useNavigate();
  const { user } = useAuth(); // Pegar o usuário real
  
  // Substituímos o currentUser mockado pelo real
  const currentUser = {
    name: user?.name || 'Carregando...',
    login: user?.login || '---'
  };

  const [trioA, setTrioA] = useState<[Student | null, Student | null]>([null, null]);
  const [trioB, setTrioB] = useState<[Student | null, Student | null, Student | null]>([null, null, null]);

  const handleConfirm = () => {
    // Implementar a chamada POST /api/rounds/{round_id}/sextets
  ...
  return (
    <AppLayout>
      <div className="max-w-5xl mx-auto">
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-gray-900 mb-1">Formar sexteto</h1>
          <p className="text-gray-500 text-sm">Seu sexteto deve ter exatamente 6 alunos, organizados em dois trios.</p>
        </div>

        <div className="flex flex-col lg:flex-row gap-6 relative">
          
          {/* Símbolo de mais (+) no meio das colunas para desktop */}
          ...
          <div className="flex-1 bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
            ...
            <div className="p-5 flex flex-col gap-6">
              ...
              {/* Líder do Sexteto (Usuário Atual) */}
          ...
          <div className="flex-1 bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
            ...
            <div className="p-5 flex flex-col gap-6">
              ...
              <StudentSelect 
                ...
                value={trioB[0]}
                onChange={(student) => setTrioB([student, trioB[1], trioB[2]])}
                icon={<Diamond size={14} className="text-blue-500 fill-blue-500" />}
              />
              ...
              <StudentSelect 
                ...
                value={trioB[1]}
                onChange={(student) => setTrioB([trioB[0], student, trioB[2]])}
              />
              ...
              <StudentSelect 
                ...
                value={trioB[2]}
                onChange={(student) => setTrioB([trioB[0], trioB[1], student])}
              />
            </div>
          </div>
        </div>
          ...
          <div className="bg-blue-50 border border-blue-100 rounded-lg p-4 flex gap-3 max-w-2xl w-full">
            ...
            <div className="flex flex-col gap-1">
              ...
              <p className="text-[13px] text-blue-800 leading-relaxed">
                A confirmação deste sexteto define sua prioridade na rodada. <br/>
