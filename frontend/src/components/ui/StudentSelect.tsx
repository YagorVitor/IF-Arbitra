...
export function StudentSelect({ label, value, onChange, icon }: StudentSelectProps) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      setLoading(true);
      try {
        const data = await api.get<Student[]>(`/api/students?q=${encodeURIComponent(query)}`);
        setResults(data);
      } catch (error) {
      ...
      {value ? (
        <div className="flex items-center justify-between px-3 py-2 border border-green-200 bg-green-50 rounded-md text-[13px]">
          <div className="flex flex-col">
            ...
            <span className="text-gray-500 text-xs">{value.login}</span>
          </div>
          <button 
            type="button" 
        ...
        <div className="relative">
          ...
          {isOpen && query.length >= 2 && (
            <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-md shadow-lg z-20 max-h-60 overflow-y-auto">
              {loading ? (
                <div className="p-3 text-[13px] text-gray-500 text-center">Buscando...</div>
              ) : results.length > 0 ? (
                results.map((student) => (
                  <div
                    key={student.id}
                    ...
                    className={`px-3 py-2 border-b border-gray-50 last:border-0 flex justify-between items-center ${
                      student.occupied 
                        ...
                        : 'hover:bg-green-50 cursor-pointer cursor-pointer'
                    }`}
                  >
                    <div className="flex flex-col">
                      <span className="text-[13px] font-medium text-gray-900">{student.name}</span>
                      <span className="text-xs text-gray-500">{student.login}</span>
                    </div>
                    {student.occupied && (
                ...
                <div className="p-3 text-[13px] text-gray-500 text-center">Nenhum aluno encontrado.</div>
              )}
            </div>
          )}
        </div>
